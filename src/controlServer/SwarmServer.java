...
public class SwarmServer {
	...
	static final int MAXIMUM_ACTIVITY_TIME_LIMIT = 600000; // 10 Minutes =
															// 600,000
	 ...
	 * are responsible for a dealing with a single client and processing its
	 * messages.
	 */
	private static class Handler extends Thread {
		 ...
		 * nameType Then runs the Rover environment simulator server process
		 */
		public void run() {
			try {
				...
				while (true) {
					...
					System.out.println(roverNameString
				...
				while (roversAreGO) {
					...
					long roverServerRequestsPerSecond = rover
							.getRoverRequestCount();
					...
					if (input.startsWith("MOVE")) {
						...
						Coord roverPos = roverLocations.getLocation(rover
								.getRoverName());
						xpos = roverPos.xpos;
						ypos = roverPos.ypos;
						...
						out.println("START_LOC " + startPos.xpos + " "
								+ startPos.ypos);
						...
						out.println("TARGET_LOC " + targetPos.xpos + " "
								+ targetPos.ypos);
						...
						int timeRemaining = 0;
						timeRemaining = (MAXIMUM_ACTIVITY_TIME_LIMIT - (int) (System
						...
						synchronized (scienceLocations) {
							...
							if (scienceLocations.checkLocation(roverPos)
									&& (rover.getRoverLastGatherTime()
											+ GATHER_TIME_PER_TILE < (System
												.currentTimeMillis()))) {

								if ((rover.getTool_1() == RoverToolType.DRILL || (rover
										.getTool_2() == RoverToolType.DRILL)
										&& (planetMap.getTile(roverPos)
												.getTerrain() == Terrain.ROCK || planetMap
												.getTile(roverPos).getTerrain() == Terrain.GRAVEL))) {
									// remove the science from scienceLocations
									// and store in rover scienceCargo
									Science foundScience = scienceLocations
											.takeScience(roverPos);
									rover.scienceCargo.add(foundScience);
									corpCollectedScience.get(
											getCorpNumber(rover)).add(
											foundScience);
									System.out.println("SwarmServer: corp "
											+ getCorpNumber(rover)
											+ " total science = "
											+ corpCollectedScience.get(
													getCorpNumber(rover))
								...
								if ((rover.getTool_1() == RoverToolType.EXCAVATOR || (rover
										.getTool_2() == RoverToolType.EXCAVATOR)
										&& (planetMap.getTile(roverPos)
												.getTerrain() == Terrain.SOIL || planetMap
												.getTile(roverPos).getTerrain() == Terrain.SAND))) {
									// remove the science from scienceLocations
									// and store in rover scienceCargo
									Science foundScience = scienceLocations
											.takeScience(roverPos);
						...
						out.println("EQUIPMENT"); // returns command header as
													// check
		...
		private String retriveScanMap(Rover thisRover) {
			...
			Gson gson = new GsonBuilder().setPrettyPrinting()
					.enableComplexMapKeySerialization().create();
			Coord roverPos = roverLocations.getLocation(thisRover
					.getRoverName());
			...
			HashMap<Coord, Science> sciHash = scienceLocations
					.getHashMapClone();
			...
			if (thisRover.getTool_1() == RoverToolType.RADIATION_SENSOR
				...
				for (Entry<Coord, Science> entry : sciHash.entrySet()) {
					if (Objects.equals(Science.RADIOACTIVE, entry.getValue())) {
						filteredScienceLocations.put(entry.getKey(),
								Science.RADIOACTIVE);
					}
				}
			}
			...
			// surrounding area
			ScanMap scanMap = planetMap.getScanMap(roverPos, scanRange,
					roverLocations, new ScienceLocations(
							filteredScienceLocations));
	...
	// ** MOVE **
	static Coord doMove(Rover thisRover, String requestedMoveDir)
			throws Exception {
		...
		synchronized (roverLocations) {
			...
			if (thisRover.getRoverDrive() == RoverDriveType.WHEELS
					&& thisRover.getRoverLastMoveTime()
							+ WHEELS_TIME_PER_SQUARE < (System
								.currentTimeMillis())
				...
				if (requestedMoveDir.equals("N")) {
					...
					MapTile possibleMovetoTile = planetMap.getTile(xCurrentPos,
							yCurrentPos);
					if (possibleMovetoTile.getTerrain() != Terrain.ROCK
						...
						// another rover
						if (roverLocations.moveRover(thisRover.getRoverName(),
								new Coord(xCurrentPos, yCurrentPos))) {
							// if moveRover call is successful then update
							// latest move time value
							thisRover.updateMoveTime();
						} else {
							// System.out.println("no move north, rover in the way");
							// //debug status out
						}
					}
				}
				...
				if (requestedMoveDir.equals("S")) {
					...
					// check planetMap (immutable)
					MapTile moveThere = planetMap.getTile(xCurrentPos,
							yCurrentPos);
					if (moveThere.getTerrain() != Terrain.ROCK
							&& moveThere.getTerrain() != Terrain.NONE) {
						// Move to the new map square, unless occupied by
						// another rover
						if (roverLocations.moveRover(thisRover.getRoverName(),
								new Coord(xCurrentPos, yCurrentPos))) {
							...
							// System.out.println("no move south, rover in the way");
							// //debug status out
						}
					}
				}
				...
				if (requestedMoveDir.equals("E")) {
					...
					if (moveThere.getTerrain() != Terrain.ROCK
						...
						if (roverLocations.moveRover(thisRover.getRoverName(),
							...
							// System.out.println("no move east, rover in the way");
							// //debug status out
						}
					}
				}
				...
				if (requestedMoveDir.equals("W")) {
					...
					if (moveThere.getTerrain() != Terrain.NONE) {
						...
						if (roverLocations.moveRover(thisRover.getRoverName(),
							...
							// System.out.println("no move west, rover in the way");
							// //debug status out
						}
					}
				}
	...
	static void updateGUIDisplay() throws Exception {
		...
		// myWorker2.displayFullMap(roverLocations.clone(), scienceLocations,
		// planetMap);
		myWorker3.displayFullMap(roverLocations.clone(), scienceLocations,
				planetMap);
	}
	...
	private static int getCorpNumber(Rover aRover) {
		...
		if (roverNumber.equals("01") || roverNumber.equals("02")
				|| roverNumber.equals("03") || roverNumber.equals("04")
				|| roverNumber.equals("05") || roverNumber.equals("06")
				|| roverNumber.equals("07") || roverNumber.equals("08")
				|| roverNumber.equals("09")) {
			tnum = 1;

			// check for Green Corp - return int 2
		} else if (roverNumber.equals("10") || roverNumber.equals("11")
				|| roverNumber.equals("12") || roverNumber.equals("13")
				|| roverNumber.equals("14") || roverNumber.equals("15")
				|| roverNumber.equals("16") || roverNumber.equals("17")
				|| roverNumber.equals("18")) {
			tnum = 2;
		}
		return tnum;
	}
