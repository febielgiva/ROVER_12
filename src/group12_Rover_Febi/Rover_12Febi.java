 ...
 * publishing their code examples dummy
 */
...
public class Rover_12Febi{
	...
	private void run() throws IOException, InterruptedException {
		...
		try {
			...
			line = in.readLine();
			//if line is null then it means rover are not connected to the server and hence rover doesn't know the start location
			...
			//setting the current direction to north
			...
			while (true) {
				...
				out.println("LOC");
				line = in.readLine();
				//get the position 12th column and 3row
	            if (line == null) {
	            	System.out.println(rovername + " check connection to server");
	            	line = "";
	            }
				if (line.startsWith("LOC")) {
					...
					//setting up the position in the gird
